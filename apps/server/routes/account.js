...
module.exports = function (app, db) {
	...
	app.post("/signup", function (req, res) {
		if (config.features.disableSignUp === true) return res.redirect("/");
			...
			[
				...
				function sendEmail(user, done) {
					if (!config.mailer.enabled) {
						logger.error(
							"config.mailer not enabled; emailing skipped. Have you configured mailer yet?"
						);
		...
		async.waterfall(
			[
				function checkToken(done) {
					User.findOne({ verifyToken: req.params.token }).exec(
						(err, user) => {
							if (err) return done(err);

							if (!user) {
								req.flash("error", {
									msg: req.t("AccountVerificationExpired"),
								});
				...
				function sendWelcomeEmailToUser(user, done) {
					if (!config.mailer.enabled) {
						...
						const err =
							"Trying to send email without config.mailer enabled; emailing skipped. Have you configured mailer yet?";
						logger.error(err);
	...
	app.get("/passwordless/:token", function (req, res) {
		...
		async.waterfall(
			[
				function checkToken(done) {
					User.findOne({ passwordLessToken: req.params.token }).exec(
						(err, user) => {
							if (err) return done(err);

							if (!user) {
								req.flash("error", {
									msg: req.t("PasswordlessTokenExpired"),
								});
					...
					User.findOne(
						...
						function (err, user) {
							if (!user) {
								...
								return done(
									`Email address ${req.body.email} is not registered!`
								);
							}
							...
							if (user.status !== 1) {
								req.flash("error", {
									msg: req.t("UserDisabledOrDeleted"),
								});
	...
	app.get("/reset/:token", function (req, res, next) {
		...
		User.findOne({ resetPasswordToken: req.params.token })
			...
			.exec((err, user) => {
				...
				if (!user) {
					req.flash("error", {
						msg: req.t("PasswordResetTokenExpired"),
					});
	...
	app.post("/reset/:token", function (req, res, next) {
		req.assert("password", req.t("PasswordTooShort")).len(6);
		req.assert("confirm", req.t("PasswordNotMatched")).equals(
			req.body.password
		);
	...
	app.get("/unlink/:provider", function (req, res) {
		...
		if (
			!req.params.provider ||
			["facebook", "twitter", "google", "github"].indexOf(
				req.params.provider
			) === -1
		)
			return response.json(
