				...
				[
					...
					function getUser(token, done) {
						...
						User.findOne(
							...
							function (err, user) {
								if (!user) {
									req.flash("error", {
										msg: req.t("UsernameIsNotAssociated", {
											username: username,
										}),
									});
									return done(
										"Invalid username or email: " + username
									);
								}
								...
								if (user.status !== 1) {
									...
									return done(
										`User '${username} is disabled or deleted!`
									);
								}
					...
					function sendResetEmailToUser(token, user, done) {
						if (!config.mailer.enabled) {
							const err =
								"Trying to send email without config.mailer not enabled; emailing skipped. Have you configured mailer yet?";
							logger.error(err);
