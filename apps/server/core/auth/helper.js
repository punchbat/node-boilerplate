 ...
 */
module.exports.tryAuthenticateWithApiKey = function tryAuthenticatedWithApiKey(
	req,
	res,
	next
) {
 ...
 * We search `apikey` field in `headers`, `query` and `body`
 */
module.exports.isAuthenticatedOrApiKey = function isAuthenticated(
	req,
	res,
	next
) {
	...
	return function (req, res, next) {
		return module.exports.isAuthenticated(req, res, function () {
			if (
				req.user &&
				req.user.roles &&
				req.user.roles.indexOf(roleRequired) !== -1
			)
				next();
		...
		User.findOne(search, function (err, existingUser) {
			if (existingUser) {
				if (existingUser._id != req.user._id) {
					...
					req.flash("error", {
						msg: req.t("SocialIDLinkedToOtherAccount"),
					});
					return done(err);
				}
				...
				User.findById(req.user.id, function (err, user) {
					...
					user.save(function (err) {
						req.flash("info", {
							msg: req.t("AccountHasBeenLinked"),
						});
						done(err, user);
					});
				});
		...
		User.findOne(search, function (err, existingUser) {
			...
			User.findOne({ email: email }, function (err, existingEmailUser) {
				...
				if (config.features.disableSignUp === true) {
					req.flash("error", {
						msg: req.t("SignUpDisabledPleaseLogin"),
					});
					return done();
				}
