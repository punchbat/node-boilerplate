...
if (config.logging.papertrail.enabled) {
	...
	let ptTransport = new winston.transports.Papertrail(
		config.logging.papertrail
	);
...
if (config.logging.graylog.enabled) {
	console.log(
		"Graylog log transport enabled! Servers: " +
			JSON.stringify(config.logging.graylog.servers)
	);
