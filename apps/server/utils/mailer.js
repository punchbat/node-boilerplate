...
module.exports = {
	send: function (recipients, subject, body, cb) {
		...
		logger.debug(
			"Deprecated! libs/mailer is deprecated. Use Service.get('mailer') instead!"
		);
		...
		if (transporter) {
			...
			transporter.sendMail(mailOptions, (err, info) => {
				if (err) logger.warn("Unable to send email: ", err);
				else logger.info("Email message sent.", info.response);
